...
## thoughts on blog images ..
/*
todo: decide how to implement images, should url be included in blog_entry content?
todo: how will the url be obtained? Upload first, get url from S3, then link in content?
will decide later.. implement minimum blog/portfolio first with this in mind.
CREATE TABLE `Blog_Images` (
    `id` int NOT NULL AUTO_INCREMENT,
    `img_url` varchar(255) NOT NULL, ## url to get from S3
    `img_caption` varchar(255) NOT NULL,
    `alt_text` varchar(255) NOT NULL,
    `post_id` int NOT NULL,
    PRIMARY KEY (`id`),
    CONSTRAINT `Blog_Images_Blog_Entries_FK` FOREIGN KEY (`post_id`) REFERENCES `Blog_Entries` (`id`) ON DELETE CASCADE
);
 */
